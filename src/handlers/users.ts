...
const signUp = async (req: Request, res: Response) => {
  const user = await store.create(req.body.username, req.body.firstname,req.body.lastname,req.body.password);
  const token = jwt.sign({ User: user }, TOKEN_SECRET);
  res.json(token);
};
...
const updateUser = async (req:Request,res: Response) => {
  const result = await store.updateOne(req.body.firstname,req.body.lastname,req.body.password,req.body.username)
  res.json(result)
}
